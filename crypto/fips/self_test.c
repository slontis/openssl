...
static int self_test_cipher(ST_CIPHER *t)
{
    ...
    if (t->tag.data != NULL) {
        ...
        if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, t->tag.len, tag_buf)
                || !test_kat(tag_buf, t->tag.len, t->tag))
            goto err;
    }
...
static int self_test_signature(ST_SIGNATURE *t)
{
    int ret = 0;
    size_t out_len = 0;
    ...
    if (!EVP_DigestSignInit(ctx, NULL, md, NULL, pkey)
            || !EVP_DigestSignUpdate(ctx, t->msg.data, t->msg.len)
            || !EVP_DigestSignFinal(ctx, NULL, &out_len)
    ...
    if (!EVP_DigestVerifyInit(ctx, NULL, md, NULL, pkey)
            ...
            || !EVP_DigestVerifyFinal(ctx, out, out_len))
        goto err;
    ret = 1;
err:
    EVP_PKEY_free(pkey);
