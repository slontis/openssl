#include <openssl/evp.h>
#include <openssl/obj_mac.h>
#include <openssl/rand_drbg.h>
#include <openssl/rsa.h>
#include <openssl/dsa.h>
#include <string.h>
#include "internal/nelem.h"
#include "crypto/rand/rand_lcl.h"

#define NOPAD 0

/* RSA Key data indicies */
#define RSA_N    0
#define RSA_E    1
#define RSA_D    2
#define RSA_P    3
#define RSA_Q    4
#define RSA_DMP1 5
#define RSA_DMQ1 6
#define RSA_IQMP 7

/* DSA Key data indicies */
#define DSA_P    0
#define DSA_Q    1
#define DSA_G    2
#define DSA_PUB  3
#define DSA_PRIV 4

/* EC Key data indicies */
#define EC_CURVE 0
#define EC_D     1
#define EC_X     2
#define EC_Y     3

/* Binary data */
typedef struct item_st {
    const unsigned char *data;
    int len;
} ITEM;

/* Self test data for a cipher - last fields required for authenticated modes */
typedef struct st_cipher_st {
    int nid;
    int pad;
    ITEM pt;
    ITEM ct_ka;
    ITEM key;
    ITEM iv;
    ITEM add;
    ITEM tag;
} ST_CIPHER;

/* Self test data for a digest */
typedef struct st_digest_st {
    int nid;
    ITEM pt;
    ITEM digest_ka;
} ST_DIGEST;

/* Self test data for a signature */
typedef struct st_sig_st {
    int key_nid;
    int digest_nid;
    ITEM *key;
    ITEM msg;
    ITEM sig_ka;
} ST_SIGNATURE;

/* Self test data for a DRBG */
typedef struct st_drbg_st {
    int nid;
    int flags;
    ITEM init_entropy;
    ITEM pers_str;
    ITEM gen_addin;
    ITEM gen_ka;
    ITEM reseed_entropy;
    ITEM reseed_addin;
    ITEM reseed_ka;
} ST_DRBG;


/* Macros to build Self test data */
#define ITM(x) { x, sizeof(x) }
#define ITM_STR(x) { x, sizeof(x) - 1 }

#define CIPHER_DATA(type, pad) { \
    NID_##type, \
    pad, \
    ITM_STR(type##_pt), \
    ITM(type##_ct), \
    ITM(type##_key), \
    ITM(type##_iv), \
}

#define CIPHER_AEAD_DATA(type, pad) { \
    NID_##type, \
    pad, \
    ITM(type##_pt), \
    ITM(type##_ct), \
    ITM(type##_key), \
    ITM(type##_nonce), \
    ITM(type##_adata), \
    ITM(type##_tag) \
}

#define DIGEST_DATA(type) { \
    NID_##type, \
    ITM(type##_pt), \
    ITM(type##_digest), \
}

#define DRBG_DATA(type, name, flags) { \
    NID_##type, flags, \
    ITM(name##_init_entropy), \
    ITM(name##_pers_str), \
    ITM(name##_addin), \
    ITM(name##_gen_kat), \
    ITM(name##_reseed_addin), \
    ITM(name##_reseed_entropy), \
    ITM(name##_reseed_kat) \
}

/* Cipher test data */

/* AES Test vector from RFC 3602 */
static const unsigned char aes_128_cbc_pt[] = "Single block msg";
static const unsigned char aes_128_cbc_key[16] =
{
    0x06, 0xa9, 0x21, 0x40, 0x36, 0xb8, 0xa1, 0x5b,
    0x51, 0x2e, 0x03, 0xd5, 0x34, 0x12, 0x00, 0x06
};
static const unsigned char aes_128_cbc_iv[16] =
{
    0x3d, 0xaf, 0xba, 0x42, 0x9d, 0x9e, 0xb4, 0x30,
    0xb4, 0x22, 0xda, 0x80, 0x2c, 0x9f, 0xac, 0x41
};
static const unsigned char aes_128_cbc_ct[16] =
{
    0xe3, 0x53, 0x77, 0x9c, 0x10, 0x79, 0xae, 0xb8,
    0x27, 0x08, 0x94, 0x2d, 0xbe, 0x77, 0x18, 0x1a
};

/* AES 128 KW data */
static const unsigned char aes_128_wrap_kek[] =
{
    0x21, 0x53, 0xcf, 0x12, 0xa3, 0x77, 0x1a, 0xc1,
    0x0c, 0x97, 0x40, 0xf3, 0x2f, 0x01, 0x07, 0x68
};

static const unsigned char aes_128_wrap_key[] =
{
    0x9a, 0x0a, 0x30, 0x9b, 0x5a, 0x96, 0x07, 0xf0,
    0xa9, 0x23, 0x5e, 0xf1, 0xfa, 0xdc, 0x1a, 0x60
};

static const unsigned char aes_128_wrapped_key[] =
{
    0x11, 0x1a, 0x8a, 0x2a, 0x01, 0x55, 0x90, 0x3b,
    0xe7, 0x36, 0xc4, 0x16, 0x2c, 0x10, 0x53, 0xb5,
    0x47, 0xc2, 0x23, 0x9b, 0xd2, 0xda, 0x50, 0x39
};


/*
 * Test vector from NIST DVPT128.rsp
 * [Alen = 32, Plen = 24, Nlen = 7, Tlen = 4]
 */
static const unsigned char aes_128_ccm_pt[24] =
{
    0xa2, 0x65, 0x48, 0x0c, 0xa8, 0x8d, 0x5f, 0x53,
    0x6d, 0xb0, 0xdc, 0x6a, 0xbc, 0x40, 0xfa, 0xf0,
    0xd0, 0x5b, 0xe7, 0xa9, 0x66, 0x97, 0x77, 0x68
};
static const unsigned char aes_128_ccm_key[16] =
{
    0xf9, 0xfd, 0xca, 0x4a, 0xc6, 0x4f, 0xe7, 0xf0,
    0x14, 0xde, 0x0f, 0x43, 0x03, 0x9c, 0x75, 0x71
};
static const unsigned char aes_128_ccm_nonce[7] =
{
    0x5a, 0x8a, 0xa4, 0x85, 0xc3, 0x16, 0xe9
};
static const unsigned char aes_128_ccm_adata[32] =
{
    0x37, 0x96, 0xcf, 0x51, 0xb8, 0x72, 0x66, 0x52,
    0xa4, 0x20, 0x47, 0x33, 0xb8, 0xfb, 0xb0, 0x47,
    0xcf, 0x00, 0xfb, 0x91, 0xa9, 0x83, 0x7e, 0x22,
    0xec, 0x22, 0xb1, 0xa2, 0x68, 0xf8, 0x8e, 0x2c
};
static const unsigned char aes_128_ccm_ct[] =
{
    0x6b, 0xe3, 0x18, 0x60, 0xca, 0x27, 0x1e, 0xf4,
    0x48, 0xde, 0x8f, 0x8d, 0x8b, 0x39, 0x34, 0x6d,
    0xaf, 0x4b, 0x81, 0xd7, 0xe9, 0x2d, 0x65, 0xb3,
};

static const unsigned char aes_128_ccm_tag[4] =
{
    0x38, 0xf1, 0x25, 0xfa
};


/* AES-GCM test data from NIST public test vectors */
static const unsigned char aes_256_gcm_key[] =
{
    0xee,0xbc,0x1f,0x57,0x48,0x7f,0x51,0x92,0x1c,0x04,0x65,0x66,
    0x5f,0x8a,0xe6,0xd1,0x65,0x8b,0xb2,0x6d,0xe6,0xf8,0xa0,0x69,
    0xa3,0x52,0x02,0x93,0xa5,0x72,0x07,0x8f
};
static const unsigned char aes_256_gcm_nonce[] =
{
    0x99,0xaa,0x3e,0x68,0xed,0x81,0x73,0xa0,0xee,0xd0,0x66,0x84
};
static const unsigned char aes_256_gcm_pt[] =
{
    0xf5,0x6e,0x87,0x05,0x5b,0xc3,0x2d,0x0e,0xeb,0x31,0xb2,0xea,
    0xcc,0x2b,0xf2,0xa5
};
static const unsigned char aes_256_gcm_adata[] =
{
    0x4d,0x23,0xc3,0xce,0xc3,0x34,0xb4,0x9b,0xdb,0x37,0x0c,0x43,
    0x7f,0xec,0x78,0xde
};
static const unsigned char aes_256_gcm_ct[] = {
    0xf7,0x26,0x44,0x13,0xa8,0x4c,0x0e,0x7c,0xd5,0x36,0x86,0x7e,
    0xb9,0xf2,0x17,0x36
};
static const unsigned char aes_256_gcm_tag[] = {
    0x67,0xba,0x05,0x10,0x26,0x2a,0xe4,0x87,0xd7,0x37,0xee,0x62,
    0x98,0xf7,0x7e,0x0c
};



/* DES3 test data */
static const unsigned char des_ede3_cbc_pt[] = "";
static const unsigned char des_ede3_cbc_key[] =
{
         0x00
};
static const unsigned char des_ede3_cbc_ct[] =
{
        0x00
};
static const unsigned char des_ede3_cbc_iv[] =
{
    0x00
};




/* Digest Data */
static const unsigned char sha1_pt[] =
{
        0x00
};
static const unsigned char sha1_digest[] =
{
        0x00
};
static const unsigned char sha512_pt[] =
{
        0x00
};
static const unsigned char sha512_digest[] =
{
        0x00
};
static const unsigned char sha3_256_pt[] =
{
        0x00
};
static const unsigned char sha3_256_digest[] =
{
        0x00
};

/* DSA Key Data */
static const unsigned char dsa_2048_p[] = {
    
};

static const unsigned char dsa_2048_q[] = {
    0xd2,0x77,0x04,0x4e,0x50,0xf5,0xa4,0xe3,0xf5,0x10,0xa5,0x0a,
    0x0b,0x84,0xfd,0xff,0xbc,0xa0,0x47,0xed,0x27,0x60,0x20,0x56,
    0x74,0x41,0xa0,0xa5
};

static const unsigned char dsa_2048_g[] = {
    
};

static const unsigned char dsa_2048_pub_key[] = {
    
};
static const unsigned char dsa_2048_priv_key[] = {
    0x0c,0x4b,0x30,0x89,0xd1,0xb8,0x62,0xcb,0x3c,0x43,0x64,0x91,
    0xf0,0x91,0x54,0x70,0xc5,0x27,0x96,0xe3,0xac,0xbe,0xe8,0x00,
    0xec,0x55,0xf6,0xcc
};

static ITEM key_dsa_2048[] =
{
    ITM(dsa_2048_p),
    ITM(dsa_2048_q),
    ITM(dsa_2048_g),
    ITM(dsa_2048_pub_key),
    ITM(dsa_2048_priv_key)
};

/* ECDSA key data */
static const unsigned char ecd_d[] =
{
    0x98,0x1f,0xb5,0xf1,0xfc,0x87,0x1d,0x7d,0xde,0x1e,0x01,0x64,
    0x09,0x9b,0xe7,0x1b,0x9f,0xad,0x63,0xdd,0x33,0x01,0xd1,0x50,
    0x80,0x93,0x50,0x30
};
static const unsigned char ecd_x[] =
{
    0x95,0x47,0x99,0x44,0x29,0x8f,0x51,0x39,0xe2,0x53,0xec,0x79,
    0xb0,0x4d,0xde,0x87,0x1a,0x76,0x54,0xd5,0x96,0xb8,0x7a,0x6d,
    0xf4,0x1c,0x2c,0x87
};
static const unsigned char ecd_y[] =
{
    0x91,0x5f,0xd5,0x31,0xdd,0x24,0xe5,0x78,0xd9,0x08,0x24,0x8a,
    0x49,0x99,0xec,0x55,0xf2,0x82,0xb3,0xc4,0xb7,0x33,0x68,0xe4,
    0x24,0xa9,0x12,0x82
};

static const unsigned char ecd_curve[] = SN_secp224r1;

static ITEM key_ecdsa_p224[] =
{
    ITM_STR(ecd_curve),
    ITM(ecd_d),
    ITM(ecd_x),
    ITM(ecd_y)
};


/* RSA key data */
static const unsigned char rsa_2048_n[] =
{

};

static const unsigned char rsa_2048_e[] =
{
    0x01, 0x00, 0x01
};

static const unsigned char rsa_2048_d[] =
{
    
};
static const unsigned char rsa_2048_p[] = {
    
};
static const unsigned char rsa_2048_q[] = {
    
};

static const unsigned char rsa_2048_dmp1[] = {
    
};
static const unsigned char rsa_2048_dmq1[] = {
    
};
static const unsigned char rsa_2048_iqmp[] = {
    
};

static ITEM key_rsa_2048[] =
{
    ITM(rsa_2048_n),
    ITM(rsa_2048_e),
    ITM(rsa_2048_d),
    ITM(rsa_2048_p),
    ITM(rsa_2048_q),
    ITM(rsa_2048_dmp1),
    ITM(rsa_2048_dmq1),
    ITM(rsa_2048_iqmp)
};

static unsigned char rsa_msg[] = "OpenSSL FIPS 140-2 Public Key RSA KAT";
/* RSA PKCS1 SHA256 */
static unsigned char rsa_sig[] =
{
    ,
};

static unsigned char dsa_msg[] = "OpenSSL FIPS 140-2 Public Key DSA KAT";

/*
 * DRBG Data
 */
static unsigned char drbg_aes_128_ctr_init_entropy[] =
{
    0x92, 0x89, 0x8f, 0x31, 0xfa, 0x1c, 0xff, 0x6d,
    0x18, 0x2f, 0x26, 0x06, 0x43, 0xdf, 0xf8, 0x18,
    0xc2, 0xa4, 0xd9, 0x72, 0xc3, 0xb9, 0xb6, 0x97
};

static unsigned char drbg_aes_128_ctr_pers_str[] =
{
    0xea, 0x65, 0xee, 0x60, 0x26, 0x4e, 0x7e, 0xb6,
    0x0e, 0x82, 0x68, 0xc4, 0x37, 0x3c, 0x5c, 0x0b
};
static unsigned char drbg_aes_128_ctr_addin[] =
{
    0x1a, 0x40, 0xfa, 0xe3, 0xcc, 0x6c, 0x7c, 0xa0,
    0xf8, 0xda, 0xba, 0x59, 0x23, 0x6d, 0xad, 0x1d
};
static unsigned char drbg_aes_128_ctr_reseed_addin[] =
{
    0x9f, 0x72, 0x76, 0x6c, 0xc7, 0x46, 0xe5, 0xed,
    0x2e, 0x53, 0x20, 0x12, 0xbc, 0x59, 0x31, 0x8c
};
static unsigned char drbg_aes_128_ctr_reseed_entropy[] =
{
    0x3d, 0xb0, 0xf0, 0x94, 0xf3, 0x05, 0x50, 0x33,
    0x17, 0x86, 0x3e, 0x22, 0x08, 0xf7, 0xa5, 0x01
};
static unsigned char drbg_aes_128_ctr_gen_kat[] =
{
    0x37, 0x19, 0xfe, 0x70, 0x32, 0xb7, 0x17, 0x83,
    0x75, 0xcb, 0x21, 0xae, 0x6e, 0xd0, 0x52, 0xde
};
static unsigned char drbg_aes_128_ctr_reseed_kat[] =
{
    0xd1, 0x41, 0x3f, 0x90, 0x50, 0x31, 0xa6, 0xb2,
    0x74, 0x73, 0xdc, 0x1a, 0xdb, 0xcf, 0x67, 0xdd
};

/*
 * Test lists
 */
ST_CIPHER cipher_tests[] =
{
    { NID_id_aes128_wrap, NOPAD,
      ITM(aes_128_wrap_key),
      ITM(aes_128_wrapped_key),
      ITM(aes_128_wrap_kek)
    },
    CIPHER_DATA(aes_128_cbc, NOPAD),
    CIPHER_DATA(des_ede3_cbc, NOPAD),
    CIPHER_AEAD_DATA(aes_256_gcm, NOPAD),
    CIPHER_AEAD_DATA(aes_128_ccm, NOPAD),
};

ST_DIGEST digest_tests[] =
{
    DIGEST_DATA(sha1),
    DIGEST_DATA(sha512),
    DIGEST_DATA(sha3_256)
};

static ST_SIGNATURE signature_tests[] =
{
    /*
     * DSA can only use a KAT if the entropy is hardwired to produce a
     * known value for k
     */
    { NID_dsa, NID_sha512, key_dsa_2048, ITM_STR(dsa_msg) },
    { NID_ecdsa_with_Specified, NID_sha256, key_ecdsa_p224, ITM_STR(dsa_msg) },
    { NID_rsa, NID_sha256, key_rsa_2048, ITM_STR(rsa_msg), ITM(rsa_sig) }
};

static ST_DRBG drbg_tests[] =
{
    DRBG_DATA(aes_128_ctr, drbg_aes_128_ctr, 0)
};
