...
/* If this flag is set then XOF mode is used (i.e right_encode(0)) */
...
static EVP_MAC_IMPL *kmac_new(void)
{
    EVP_MAC_IMPL *kctx;
...
static int kmac_init(EVP_MAC_IMPL *kctx)
{
    ...
    /* Only check for key here (setting the key is dependent on the digest) */
    if (kctx->key_len == 0) {
        EVPerr(EVP_F_KMAC_INIT, EVP_R_NO_KEY_SET);
        return 0;
    }
    block_len = EVP_MD_CTX_block_size(ctx);
...
static int kmac_final(EVP_MAC_IMPL *kctx, unsigned char *out)
{
    ...
    if ((kctx->flags & KMAC_FLAG_XOF_MODE) != 0)
        lbits = 0;
    else
        lbits = kctx->out_len * 8;
 ...
 *           right_encoded(0) is part of the digested data, otherwise it uses
...
static int kmac_ctrl(EVP_MAC_IMPL *kctx, int cmd, va_list args)
{
    ...
    case EVP_MAC_CTRL_SET_MD:
        ...
        if (md != EVP_kmac128() && md != EVP_kmac256()) {
            EVPerr(EVP_F_KMAC_CTRL, EVP_R_INVALID_DIGEST);
            return 0;
        }
        kctx->out_len = md->md_size;
        return EVP_DigestInit_ex(kctx->ctx, md, NULL);
    case EVP_MAC_CTRL_SET_FLAGS:
        flags = va_arg(args, size_t);
        if (flags == 1)
            kctx->flags |= KMAC_FLAG_XOF_MODE;
        else
            kctx->flags &= ~KMAC_FLAG_XOF_MODE;
        return 1;
    ...
    case EVP_MAC_CTRL_SET_KEY:
        ...
        /* The key encoding relies on md being set first */
        if (EVP_MD_CTX_md(ctx) == NULL) {
            EVPerr(EVP_F_KMAC_CTRL, EVP_R_NO_DIGEST_SET);
            return 0;
        }
                         ...
                         const char *value)
{
    ...
    if (strcmp(type, "digest") == 0) {
        ...
        if (md == NULL)
            return 0;
        return kmac_ctrl_int(kctx, EVP_MAC_CTRL_SET_MD, md);
    }
    if (strcmp(type, "outlen") == 0)
        return kmac_ctrl_int(kctx, EVP_MAC_CTRL_SET_SIZE, atoi(value));
    if (strcmp(type, "xof") == 0)
        return kmac_ctrl_int(kctx, EVP_MAC_CTRL_SET_FLAGS, atoi(value));
...
{
    unsigned int cnt, sz = sizeof(size_t);

    for (cnt = 0; bits && (cnt < sz); ++cnt, bits >>= 8) {
    }
