                   ...
                   const unsigned char *in3, size_t in3len)
{
    ...
    tmp[1] = (num_bits_returned >> 24) & 0xff;
    tmp[2] = (num_bits_returned >> 16) & 0xff;
    tmp[3] = (num_bits_returned >> 8) & 0xff;
    tmp[4] = num_bits_returned & 0xff;
    ...
    /* (Step 4) */
    for (;;) {
        /* (Step 4.1) temp = temp || Hash(counter||num_bits_returned||input */
                     ...
                     unsigned char *in, size_t inlen)
{
    ...
    if (drbg->seedlen < 1 || inlen < 1 || inlen > drbg->seedlen)
        return 0;
                         ...
                         const unsigned char *adin, size_t adinlen)
{
    ...
    return EVP_DigestInit_ex(ctx, hash->md, NULL)
           ...
           && EVP_DigestFinal(hash->ctx, hash->vtmp, NULL)
                              ...
                              const unsigned char *adin, size_t adin_len)
{
    ...
    int reseed_counter = drbg->reseed_counter;
    counter[0] = (reseed_counter >> 24) & 0xff;
    counter[1] = (reseed_counter >> 16) & 0xff;
    counter[2] = (reseed_counter >> 8) & 0xff;
    counter[3] = reseed_counter & 0xff;
...
int drbg_hash_init(RAND_DRBG *drbg)
{
    ...
    drbg->max_request = 1 << 16;           /* max is 2^19 bits */
