...
int ossl_sha3_final(unsigned char *md, KECCAK1600_CTX *ctx)
{
    ...
    ctx->xof_state = XOF_STATE_FINAL;
    SHA3_squeeze(ctx->A, md, ctx->md_size, bsz, 0);
    return 1;
}
 ...
 */
int ossl_sha3_squeeze(unsigned char *md, KECCAK1600_CTX *ctx)
{
    size_t outlen = ctx->md_size;
    size_t bsz = ctx->block_size;
    size_t num = ctx->bufsz;
    size_t len;
    int next = 1;
    ...
    if (num != 0) {
        if (outlen > ctx->bufsz)
            ...
            len = outlen;
        memcpy(md, ctx->buf + bsz - ctx->bufsz, len);
        md += len;
        outlen -= len;
        ctx->bufsz -= len;
    }
    ...
    if (outlen >= bsz) {
        len = bsz * (outlen / bsz);
        SHA3_squeeze(ctx->A, md, len, bsz, next);
        next = 1;
        md += len;
        outlen -= len;
    }
