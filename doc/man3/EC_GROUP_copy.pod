...
EC_GROUP_check_named_curve checks that a named curve's domain parameters have not been modified.
If B<nist_only> is 1 it will only look for NIST approved curves, otherwise it searches all builtin curves. This method can
be used to find the name of a curve that has been loaded explicitly without a curve name.
...
EC_GROUP_check_named_curve returns the nid of the named curve if the parameters are unmodified, otherwise it returns 0.
