...
EVP_SignFinal, EVP_SignFinal_with_libctx
...
EVP_SignFinal_with_libctx() is similar to EVP_SignFinal() but also passes the
library context I<libctx> and the property query I<propq> to use when creating
a context to use with the key I<pkey>. EVP_SignFinal() uses NULL values for
these fields.
...
EVP_SignInit_ex(), EVP_SignUpdate(), EVP_SignFinal() and
EVP_SignFinal_with_libctx() return 1 for success and 0 for failure.
