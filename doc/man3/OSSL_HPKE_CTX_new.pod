...
to produces the encapsulated public value I<enc>.
On input I<enclen> should contain the maximum size of the I<enc> buffer, and returns
the output size. An error will occur if the input I<enclen> is too small.
...
the output size. An error will occur if the input I<ctlen> is too small.
...
returns the output size. An error will occur if the input I<ptlen> is too small.
...
supplied label I<label> of size I<lablelen>, to produce a secret I<secret>
...
plaintext of length I<clearlen>.  (AEAD algoriths add a data integrity tag,
...
OSSL_HPKE_str2suite() maps input I<str> strings to a B<OSSL_HPKE_SUITE> object.
