...
I<pub>, and an optional I<info> parameter that can bind other protocol or
application artefacts such as identifiers, and produces the encapsulated public
value I<enc>.  Generally, the encapsulated public value I<enc> corresponds to a
...
All input buffers (such as I<pub> above) are presented to these APIs using a pair of
parameters: an unsigned char* pointer I<foo> and a size_t length value called
I<foolen>. All input/output buffers (such as I<enc> above) are similarly presented 
but with the length being a pointer to a size_t that on input should have the
caller-allocated buffer size and that on successful output will have the length
of the value placed in that buffer. (If the buffer size is too small an error
will occur.)
...
If no "real" encryption or decryption operations are needed in a given
application context, then the I<suite> used when forming an OSSL_HPKE_CTX to be
used for exporting secrets can use the "fake" AEAD identifier denoted by
I<OSSL_HPKE_AEAD_ID_EXPORTONLY>.
...
with the provided I<secretlen> length.

Multiple calls to OSSL_HPKE_export() with the same OSSL_HPKE_CTX
are supported, but as there is no real encryption there is no 
incrementing nonce value so the same I<secret> output will be returned
for a given OSSL_HPKE_CTX if the same I<label> and I<secretlen> inputs 
