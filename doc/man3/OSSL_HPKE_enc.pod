...
OSSL_HPKE_get_ciphertext_size() provides a way for the caller to know by how much
cipertext is longer than plaintext. OSSL_HPKE_get_public_encap_size() provides a 
way for the caller to know how big the encapsulated public value will be for a
given HPKE suite.
...
(typically one used by a client/sender/encryptor), that key will be used for
seal operations. If not, a new ephemeral key will be generated per call and
then used for the lifetime of the OSSL_HPKE_CTX object. In almost all cases, it
is better to not provide an external private key but to rather use an
ephemerally generated key, so use of OSSL_HPKE_CTX_set1_senderpriv() is
discouraged. (This is mainly provided to allow validation of test vectors.)
...
If the "EXPORT-ONLY" mode from RFC 9180 is desired, then OSSL_HPKE_CTX_export()
