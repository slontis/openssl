...
Prior to OpenSSL 3.0 the ivlen could be smaller than the required 12 bytes using
EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, ivlen, NULL). This resulted in
an iv that was LSB zero padded to be 12 bytes.
In 3.0 setting the iv length to any value other than 12 will result in an
error.
