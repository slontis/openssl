...
static int shake_absorb_test(void)
{
    ...
    for (stride = 1; stride < total; ++stride) {
        ...
        for (i = 0; i < total; i += sz) {
            ...
            if ((i + sz) > total)
                sz = total - i;
            if (!TEST_true(EVP_DigestUpdate(ctx, shake256_largemsg_input + i, sz)))
                goto err;
        }
                                 ...
                                 size_t expected_outlen)
{
    int ret = 0;
    EVP_MD_CTX *ctx = NULL;
    unsigned char out[expected_outlen];
    ...
    if (!TEST_mem_eq(out, sizeof(out), expected_out, expected_outlen))
        goto err;
    ret = 1;
err:
    EVP_MD_CTX_free(ctx);
